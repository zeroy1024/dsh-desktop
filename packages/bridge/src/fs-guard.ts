 ...
 *    纯字符串层判定（无 fs I/O）；symlink 逃逸由调用方在 fs 侧用 realpath
 *    再校验一次（两道都过才放行）。
 */
 ...
 * @param root - 会话工作目录的绝对路径（调用方保证已规范化，无尾分隔符）。
...
export function resolveWithinRoot(root: string, rel: string): string | undefined {
  ...
  if (normalized !== rootNorm && !normalized.startsWith(`${rootNorm === '/' ? '' : rootNorm}/`)) return undefined
  return normalized
}

/** POSIX 风格路径折叠（处理 `//` 与内部空段；调用方已拒绝 `.`/`..` 段，这里无需展开相对段）。 */
function normalizePath(path: string): string {
  const segments = path.split('/')
  const kept = segments.filter((segment, index) => segment !== '' || index === 0)
  return kept.join('/')
}
